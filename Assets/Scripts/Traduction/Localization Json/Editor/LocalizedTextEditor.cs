...
public class LocalizedTextEditor : EditorWindow {
    ...
    public LocalizedAssetArray localizedAssets;
    ...
    private void OnGUI()
    {
        //Debug.Log("Screen Width : " + position.width + ", Screen Height : " + position.height);
    ...
    void DrawLayouts()  //Affiche le header en lui-même
    {
        ...
        headerSectionRect = new Rect(0, 0, position.width, editorWindowStyle.headerSectionSize);
        GUI.DrawTexture(headerSectionRect, editorWindowStyle.headerSectionTexture);


        //Pour la section dictionnaire
        dicoSectionRect = new Rect(0, editorWindowStyle.headerSectionSize, position.width, position.height - editorWindowStyle.settingsSectionSize * 3f - editorWindowStyle.settingsSectionSize / 2f);
        GUI.DrawTexture(dicoSectionRect, editorWindowStyle.dicoSectionTexture);

        //Pour la section settings
        settingsSectionRect = new Rect(0, position.height - editorWindowStyle.settingsSectionSize, position.width, editorWindowStyle.settingsSectionSize);
    ...
    void DrawLogo()   //Va afficher le logo du header en bas à gauche de l'écran
    {
        ...
        Rect logoRect = new Rect(editorWindowStyle.logoScreenResX, position.height - editorWindowStyle.logoScreenResY, 
    ...
    void DrawHeader()   //Va afficher le contenu du header
    {
        ...
        GUIStyle titreHeaderStyle = new GUIStyle 
        { 
            fontStyle = editorWindowStyle.titreFontStyle,
        ...
        GUI.Label(new Rect(0, 0, position.width, headerSectionRect.height), "LocalizationManager", titreHeaderStyle);
    ...
    void DrawDico()   //Va afficher le contenu du dictionnaire
    {
        ...
        if (localizedAssets != null)
        {
            ...
            scrollPosition = EditorGUILayout.BeginScrollView(scrollPosition, GUILayout.Width(dicoSectionRect.width), GUILayout.Height(dicoSectionRect.height - 50));



            GUI.backgroundColor = editorWindowStyle.backgroundTextColor;

            GUIStyle titreDicoStyle = new GUIStyle 
            {  
                fontStyle = editorWindowStyle.sousTitreFontStyle,
            ...
            GUI.Label(new Rect(0, 5, position.width, editorWindowStyle.sousTitreFontSize), "Dictionnaire actuel : " + fileName, titreDicoStyle);
            ...
            SerializedProperty serializedProperty = serializedObject.FindProperty("localizedAssets");
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                //LocalizationManager lm = FindObjectOfType<LocalizationManager>();
                //LocalizationStartupManager lsm = FindObjectOfType<LocalizationStartupManager>();

                //if (lm)
                //    lm.LoadLocalizedText(lm.currentLanguage);

                //if (lsm)
                //    lsm.ChangeAllComponentsInScene();
            }
    ...
    void DrawSettings()
    {
        ...
        GUI.Label(new Rect(0, 0, 100, 20), "Thème :", titreHeaderStyle);
        index = EditorGUI.IntPopup(new Rect(100, 0, 100, 20), index, options, indexes);
        if (GUI.changed)
        {
            editorWindowStyle = allAvailableStyles[index];
        }
    ...
    {
        localizedAssets = new LocalizedAssetArray();
    ...
    private void SaveLocalizationData()
    {
        ...
        {
            string dataAsJson = JsonUtility.ToJson(localizedAssets, true);
    ...
    private void LoadLocalizationData()
    {
        ...
        if (!string.IsNullOrEmpty(filePath))
        {
            ...
            localizedAssets = JsonUtility.FromJson<LocalizedAssetArray>(dataAsJson);
            Repaint();
        }
    }
}
